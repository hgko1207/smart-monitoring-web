...
public class AwsService {
	...
	public WeatherInfo createWeatherInfo() {
		...
		getForecastSpaceData(weatherInfo);
		return weatherInfo;
	}
	 ...
	 * @return
	 */
	private String createUrl(String url, String date, String hour) {
		...
		try {
			...
			urlBuilder.append("&" + URLEncoder.encode("base_date", "UTF-8") + "=" + URLEncoder.encode(date, "UTF-8")); /*현재일*/
			urlBuilder.append("&" + URLEncoder.encode("base_time", "UTF-8") + "=" + URLEncoder.encode(hour, "UTF-8")); /*현재시간*/
	...
	public void getForecastSpaceData(WeatherInfo weatherInfo) {
		...
		try {
			URL url = new URL(createUrl(BASE_URL, currentDate, currentHour));
	...
	private void createWeatherInfo(WeatherInfo weatherInfo, String json) {
		...
		try {
			...
			for (ForecastSpaceData item : items.getItem()) {
				...
				}  else if (item.getCategory() == CategoryType.R06) {
			...
			weatherInfo.setDate(date + " " + currentHour.substring(0, 2) + ":00 기준");
	 ...
	 * @return
	 */
	public String getUltraSrtNcstTime() {
		Calendar calendar = Calendar.getInstance();
		calendar.add(Calendar.HOUR, -1);
		String date = hourFormat.format(calendar.getTime());
		return date + "00";
	}
}
